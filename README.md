...
共计 **298+ 页面**，包含 69 个概念页面、223 个实体页面、6 个来源摘要。
...
| 战斗系统 | 6 个战斗场景，对抗检定 + 多回合近战，弹出层 UI |
| 效果系统 | HP/SAN/MP/Luck 调整、技能成长、物品管理、检定门控效果 |
| 节点图导航 | Canvas 力导向图，可视化 270 条目跳转关系，支持平移/缩放/悬停预览 |
...
data/       ← 声明式数据（模组条目、战斗脚本、技能数据、节点图数据）
ui/         ← 渲染层（故事面板、角色面板、战斗弹出层、节点图）
```
...
│       │   ├── adapters/        # 适配层（角色卡/骰子）
│       │   ├── data/            # 数据层（模组条目/战斗脚本/技能/节点图数据）
│       │   ├── ui/              # 渲染层（故事/角色面板/战斗弹出层/节点图）
│       │   └── app.js           # 主入口

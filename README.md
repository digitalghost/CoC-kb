...
| 装备 | **~431** | 1920s ~288 件 + 现代 ~143 件，覆盖 17 个分类 |
| 武器 | **~172** | 1920s ~82 件 + 现代 ~90 件，含完整战斗属性 |
...
一个模拟实体角色卡正反面的数据展示与追踪工具，适合在游戏过程中实时管理调查员状态。
...
│   └── character-tracker/       # 调查员角色卡追踪器（单文件 HTML）
│       ├── index.html           # 全部 HTML/CSS/JS（~93KB）
│       ├── assets/              # 背景图 + 角饰 SVG
